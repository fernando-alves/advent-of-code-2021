...
const { markLinearVents, countLinearOverlapingPoints, parseLinePoints, markAllVents } = require('./navigation.js')

test('marks vents based on a single line', t => {
  const board = markLinearVents([[{x: 0, y: 9}, {x: 5, y: 9}]])
...
test('marks vents based on a single column', t => {
  const board = markLinearVents([[{x: 0, y: 9}, {x: 0, y: 15}]])
...
test('marks vents in a line with overlapping lines', t => {
  const board = markLinearVents([[{x: 0, y: 9}, {x: 5, y: 9}], [{x: 0, y: 9}, {x: 2, y: 9}]])
...
test('marks vents in diagonal', t => {
  const board = markAllVents([[{x: 9, y: 7}, {x: 7, y: 9}]])
  const allVisitedCorrectly = [{x: 9, y: 7}, {x: 8 , y: 8}, {x: 7, y: 9}].every(({x, y}) => board[x][y].lineCount == 1)
  t.is(allVisitedCorrectly, true)
})
